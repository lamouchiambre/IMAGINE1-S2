...
public class Maillage {
	...
	public void addFace1eh1p(HEdge h12, Vertex v3) {
	...
	public void addFace2a(HEdge h12, HEdge h23) {
		HEdge h31 = new HEdge(h23.getVhead(), h12, h23, null,null);
		HEdge h13 = new HEdge(h12.getOpposite().getVhead(), h23.getOpposite(), h12.getOpposite(), h31,null);
		...
		h23.setFace(f);
		this.faces.add(f);
		this.hedges.get(this.hedges.indexOf(h12)).setFace(f);
		this.hedges.get(this.hedges.indexOf(h23)).setFace(f);
		this.hedges.add(h13);
	}
	
	public void addFace2a(HEdge h31, Vertex v1, Vertex v3 ) {
		h31.setVhead(v1);
		h31.setNext(v3.getEdge().getPrec().getOpposite().getVhead().getEdge());
	} 
...
//	}
	...
	public ArrayList<HEdge> insidence(Vertex s){
		ArrayList<HEdge> list  = new ArrayList<HEdge>();
		HEdge eh = s.getEdge().getOpposite();
		while (eh.getFace() != null) {
			list.add(eh);
			list.add(eh.getOpposite());
			eh = eh.getOpposite();
			for(int i = 0; i <2; i++) {
	...
	public static void main(String[] args) {
		...
		//creation d'une tetraedre
		m.addFace3p(v1, v2, v3);
		m.addFace1eh1p(m.getHedges().get(0), v3);
		m.addFace2a(m.getHedges().get(0), m.getHedges().get(5));
