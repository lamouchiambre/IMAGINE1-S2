...
public class MeshMap implements Iterable<Dart>{
    ...
    public Boolean illegalDelaunay(Vertex r1, Vertex s, Vertex t, Vertex r2){
        ...
        // -------------------------------------------------------------------------------------------------------------
        // TODO
        Circle crst = r1.point.circumcircle(r1.point, s.point, t.point);

        Vector vOr2 = new Vector(crst.getCenter(), r2.point);

        if (vOr2.getNorm() < crst.getRadius()) {
            return true;
        }
        // -------------------------------------------------------------------------------------------------------------
        // -------------------------------------------------------------------------------------------------------------
    ...
    public void flipEdge(Dart dart){
        ...
        // -------------------------------------------------------------------------------------------------------------
        // TODO
        // Dart d_next =  dart.getNext();
        // Dart d_prev =  dart.getPrev();
        // dart.setPrev(d_next);
        // dart.setNext(dart.getOpposite().getNext());
        // // -------
        // Dart do_next = dart.getOpposite().getNext();
        // Dart do_prev = dart.getOpposite().getPrev();
        // dart.getOpposite().setNext(d_prev);
        // dart.getOpposite().setPrev(do_next);
        // // --------
        // do_prev.setNext(d_next);
        // d_prev.setNext(do_next);


        // -------------------------------------------------------------------------------------------------------------
        // -------------------------------------------------------------------------------------------------------------
        System.out.println("flipEdge");
        Vertex T = dart.getStartVertex();
        Vertex S = dart.getNext().getStartVertex();
        Vertex r2 = dart.getNext().getNext().getStartVertex();
        Vertex r1 = dart.getOpposite().getNext().getEndVertex();

        Dart prev = dart.getPrev();
        Dart next = dart.getNext();
        Dart opp = dart.getOpposite();
        Dart oppNext = dart.getOpposite().getNext();
        Dart oppPrev = dart.getOpposite().getPrev();

        dart.setStartVertex(r1);
        dart.setEndVertex(r2);
        dart.setPrev(oppNext);
        dart.setNext(prev);

        opp.setStartVertex(r2);
        opp.setEndVertex(r1);
        opp.setNext(oppPrev);
        opp.setPrev(next);

        prev.setNext(oppNext);
        oppPrev.setNext(next);

        next.setNext(opp);
        next.face = opp.face;
        oppNext.setNext(dart);
        oppNext.face=dart.face;

        dart.face.refDart=dart;
        opp.face.refDart=opp;
    ...
    public void delaunayTriangulate(){
        ...
        do {
            Dart st = pop(stack);
            if (illegalDelaunay(st.getOpposite().getNext().getEndVertex(), st.getStartVertex(), st.getEndVertex(), st.getNext().getEndVertex())) {
                flipEdge(st);
                ArrayList<Dart> tmp =  new ArrayList<Dart>();
                tmp.add(st.getNext());
                tmp.add(st.getPrev());
                tmp.add(st.getOpposite().getNext());
                tmp.add(st.getOpposite().getPrev());
